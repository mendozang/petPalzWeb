...
@Component({
  ...
  styleUrl: './monitoreo-salud.component.scss'
})
export class MonitoreoSaludComponent implements AfterViewInit, OnChanges{
  isBrowser = false;
  @Input() petId!: number;
  pet: any;
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['PetId'] && changes['PetId'].currentValue) {
      this.clearCharts();
      this.fetchPetDetails(this.petId);
    }
  }
  ...
  fetchHealthData(petId: number): void {
    ...
    this.http.get(`https://petpalzapi.onrender.com/api/Monitoreo/mascota/${this.petId}`).subscribe(
      (response: any) => {
